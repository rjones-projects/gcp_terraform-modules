locals {
  gke_standard_clusters = {
    for spec in try(var.gke_standard_cluster.spec, []) : try(spec.service_name, var.gke_standard_cluster_default.service_name) => {
      ...
      master_authorized_networks           = try(spec.master_authorized_networks, var.gke_standard_cluster_default.master_authorized_networks)
      enable_dns_endpoint                  = try(spec.enable_dns_endpoint, var.gke_standard_cluster_default.enable_dns_endpoint)
      dns_endpoint_allow_external_traffic  = try(spec.dns_endpoint_allow_external_traffic, var.gke_standard_cluster_default.dns_endpoint_allow_external_traffic)
      enable_k8s_tokens_via_dns            = try(spec.enable_k8s_tokens_via_dns, var.gke_standard_cluster_default.enable_k8s_tokens_via_dns)
      enable_k8s_certs_via_dns             = try(spec.enable_k8s_certs_via_dns, var.gke_standard_cluster_default.enable_k8s_certs_via_dns)
      ...
      enable_secret_manager_addon          = try(spec.enable_secret_manager_addon, var.gke_standard_cluster_default.enable_secret_manager_addon)
      secret_sync_config                   = try(spec.secret_sync_config, var.gke_standard_cluster_default.secret_sync_config)
