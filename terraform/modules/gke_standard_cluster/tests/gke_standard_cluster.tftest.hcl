...
variables {
  ...
  gke_standard_cluster = {
    spec = [
      {
        service_name               = "test-standard-cluster"
        zone                       = "europe-west2-a"
        ...
        service_account_email      = "gke-nodes-sa@test-project-id.iam.gserviceaccount.com"
...
run "plan_basic_standard_cluster" {
  command = plan

  assert {
    condition     = length(output.names) == 1
    error_message = "Expected 1 standard cluster"
  }
}
